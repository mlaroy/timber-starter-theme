...
function mlaroy_stylesheet_uri($stylesheet_uri, $stylesheet_dir_uri){
	return $stylesheet_dir_uri . '/dist/app.css';
}
add_filter('stylesheet_uri', 'mlaroy_stylesheet_uri', 10, 2);

/**
 * Use wp_enqueue to add theme stylesheet & js to wp_head()
 */

function mlaroy_enqueue_styles_scripts() {
	...
	wp_enqueue_script( 'application', get_template_directory_uri() . '/dist/app.js', array(), '1.0.0', true );
...
add_action( 'wp_enqueue_scripts', 'mlaroy_enqueue_styles_scripts', 15 );